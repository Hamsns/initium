...
export default function HomePage() {
  ...
  const research = Array.from({ length: 3 }).map((_, i) => ({ title: `Paper Title #${i + 1}`, href: "/research" }));
  const blogs = Array.from({ length: 3 }).map((_, i) => ({ title: `Post Title #${i + 1}`, href: "/blog" }));

  return (
    <>
      {/* INITIUM + divider + tagline */}
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-10">
        <div className="grid md:grid-cols-12 gap-8 items-end">
          ...
          <div className="hidden md:flex md:col-span-1 items-end">
            <div className="w-[2px] bg-[#3498DB]" style={{ height: "72px", marginBottom: "8px" }} aria-hidden />
          </div>
        ...
        {/* Space between header and gradient */}
        <div className="mt-6" />

        {/* Gradient hero with the ONLY tagline ribbon */}
        <GradientHero />
      </section>
        ...
        <div className="mt-8 grid md:grid-cols-3 gap-10">
          {featured.map((item, idx) => (
            <Link key={idx} href={item.href} className="group">
              ...
              <h3 className="mt-6 text-[26px] font-extrabold text-slate-900 group-hover:underline">{item.title}</h3>
              <Rule />
